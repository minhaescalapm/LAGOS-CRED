...
export function ClientForm({ onClose, onSubmit, clientToEdit }: ClientFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/84 backdrop-blur-sm animate-fade-in select-none">
      <div 
        ...
        className="w-full max-w-lg bg-zinc-900 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-2xl flex flex-col"
      >
        {/* Header */}
        ...
        {/* Form Body */}
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto max-h-[75vh] p-4 md:p-6 space-y-4">
          {error && (
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-[10px] text-zinc-400 font-medium block mb-1">Tempo (Dias úteis/corridos)</label>
                <div className="relative">
                  <Calendar className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                  <select 
                    value={totalDays}
                    onChange={e => setTotalDays(Number(e.target.value))}
                    className="w-full bg-zinc-950/60 border border-zinc-855 focus:border-amber-400 focus:outline-none rounded-xl py-2.5 pl-10 pr-3 text-sm text-zinc-100 transition-colors appearance-none cursor-pointer"
                  >
                    {[10, 15, 20, 26, 30, 45, 60].map(day => (
                      <option key={day} value={day} className="bg-zinc-900 text-zinc-200">
                        {day} dias diários
                      </option>
                    ))}
                  </select>
